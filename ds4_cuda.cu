...
/* Backend diagnostics hook (defined in ds4.c; declared with C linkage to match
...
static void *cuda_tmp_alloc(uint64_t bytes, const char *what) {
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA temp alloc failed for %s (%.2f MiB): %s\n",
...
static const char *cuda_model_range_ptr(const void *model_map, uint64_t offset, uint64_t bytes, const char *what) {
    ...
    if (err != cudaSuccess) {
        (void)cudaGetLastError();
        ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model range alloc failed for %s (%.2f MiB): %s\n",
    ...
    for (uint64_t done = 0; done < bytes; done += chunk) {
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model range copy failed for %s at %.2f/%.2f MiB: %s\n",
                    what ? what : "weights",
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA cached %s %.2f MiB (total %.2f GiB)\n",
                what ? what : "weights",
                (double)bytes / 1048576.0,
...
static int cuda_q8_f16_cache_has_budget(uint64_t request_bytes, const char *label) {
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA q8 fp16 cache memory query failed: %s; using q8 kernels\n",
                cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }
...
static const __half *cuda_q8_f16_ptr(
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA q8 fp16 cache alloc failed (%.2f MiB): %s\n",
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA cached q8 fp16 %.2f MiB (total %.2f GiB)\n",
...
static float *cuda_q8_f32_ptr(
    ...
    cudaError_t err = cudaMalloc(&dev, (size_t)out_bytes);
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA q8 fp32 cache alloc failed (%.2f MiB): %s\n",
                (double)out_bytes / 1048576.0, cudaGetErrorString(err));
        (void)cudaGetLastError();
        return NULL;
    }
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA cached q8 fp32 %.2f MiB (total %.2f GiB)\n",
                (double)out_bytes / 1048576.0,
...
static int cuda_ok(cudaError_t err, const char *what) {
    ...
    ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA %s failed: %s\n", what, cudaGetErrorString(err));
    return 0;
}
...
static void cuda_model_load_progress_note(uint64_t cached_bytes) {
    ...
    if (!g_model_load_progress_started) {
        ...
        if (g_model_load_progress_tty) {
            ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA loading model tensors into device cache: 0.00 GiB");
        } else {
    ...
    if (g_model_load_progress_tty) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "\rds4: CUDA loading model tensors into device cache: %.2f GiB",
...
static int cuda_model_prefetch_range(const void *model_map, uint64_t model_size, uint64_t map_offset, uint64_t map_size) {
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model read-mostly advise skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }
    err = cudaMemAdvise(pre_ptr, (size_t)pre_bytes, cudaMemAdviseSetPreferredLocation, loc);
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model preferred-location advise skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }
    ...
    if (!g_model_prefetch_stream) {
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model prefetch stream creation skipped: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
    }
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model prefetch skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }
    if (getenv("DS4_CUDA_MODEL_PREFETCH_SYNC") != NULL) {
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model prefetch sync failed: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
    }
...
static int cuda_model_stage_pool_alloc(uint64_t bytes) {
    ...
    if (!g_model_upload_stream) {
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model upload stream creation failed: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
    }
    for (size_t i = 0; i < 4; i++) {
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA pinned model staging allocation failed: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model staging event creation failed: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
    }
    ...
    if (g_model_direct_fd >= 0 && g_model_direct_align > 1 && g_model_file_size != 0) {
        ...
        if (aligned_off <= g_model_file_size &&
            ...
            if (direct_errno == EINVAL || direct_errno == EFAULT || direct_errno == ENOTSUP || direct_errno == EOPNOTSUPP) {
                if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
                    ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA direct model read disabled: %s\n", strerror(direct_errno));
                }
...
static char *cuda_model_arena_alloc(uint64_t bytes, const char *what) {
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model arena alloc failed for %s (%.2f MiB chunk): %s\n",
                what ? what : "weights",
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        ...
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model arena allocated %.2f MiB (arenas %.2f GiB)\n",
                (double)chunk / 1048576.0,
...
static const char *cuda_model_range_ptr_from_fd(
    ...
    if (g_model_range_bytes > limit || bytes > limit - g_model_range_bytes) {
        if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
            ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA direct %s %.2f MiB (cache budget %.2f GiB exhausted)\n",
                    what ? what : "weights",
                    (double)bytes / 1048576.0,
    ...
    while (copied < bytes) {
        ...
        if (chunk_idx >= 4u) {
            ...
            if (err != cudaSuccess) {
                ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model staging wait failed for %s: %s\n",
                        what ? what : "weights", cudaGetErrorString(err));
                (void)cudaGetLastError();
                return NULL;
            }
        ...
        if (!cuda_model_stage_read(g_model_stage[bi], g_model_stage_bytes,
            ...
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model range read failed for %s at %.2f MiB: %s\n",
                    what ? what : "weights",
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model range copy failed for %s at %.2f MiB: %s\n",
                    what ? what : "weights",
                    (double)copied / 1048576.0,
                    cudaGetErrorString(err));
            (void)cudaGetLastError();
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model staging record failed for %s: %s\n",
                    what ? what : "weights", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return NULL;
        }
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model range upload sync failed for %s: %s\n",
                what ? what : "weights", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return NULL;
    }
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA fd-cached %s %.2f MiB (total %.2f GiB)\n",
                what ? what : "weights",
                (double)bytes / 1048576.0,
                (double)g_model_range_bytes / 1073741824.0);
    }
...
static int cuda_model_copy_chunked(const void *model_map, uint64_t model_size, uint64_t map_offset, uint64_t map_size) {
    ...
    if (err != cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model allocation skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }

    ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA chunk-copying %.2f GiB model image\n",
    ...
    if (map_offset > 0) {
        ...
        while (copied_header < map_offset) {
            ...
            if (err != cudaSuccess) {
                ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model header copy failed: %s\n", cudaGetErrorString(err));
    ...
    while (copied < map_size) {
        ...
        if (err != cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model chunk copy failed at %.2f GiB: %s\n",
        ...
        if (getenv("DS4_CUDA_MODEL_COPY_VERBOSE") != NULL && now - last_report >= 2.0) {
            ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model chunk copy %.2f/%.2f GiB\n",
...
static int cublas_ok(cublasStatus_t st, const char *what) {
    ...
    ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: cuBLAS %s failed: status %d\n", what, (int)st);
    return 0;
}
...
extern "C" int ds4_gpu_init(void) {
    ...
    if (cudaGetDeviceProperties(&prop, dev) == cudaSuccess) {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA backend initialized on %s (sm_%d%d)\n",
...
extern "C" int ds4_gpu_set_model_map(const void *model_map, uint64_t model_size) {
    ...
    if (copy_env && copy_env[0]) {
        ...
        cudaError_t err = cudaMalloc(&dev, (size_t)model_size);
        if (err == cudaSuccess) {
            ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA copying %.2f GiB model to device memory\n",
                    (double)model_size / 1073741824.0);
            ...
            if (err == cudaSuccess) {
                ...
                ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model copy complete in %.3fs\n", t1 - t0);
                return 1;
            }
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model copy failed: %s\n", cudaGetErrorString(err));
            (void)cudaFree(dev);
            (void)cudaGetLastError();
        } else {
            ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model allocation skipped: %s\n", cudaGetErrorString(err));
    ...
    if (err == cudaSuccess) {
        ...
        if (err == cudaSuccess && dev) {
            ...
            ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA registered %.2f GiB model mapping for device access\n",
                    (double)model_size / 1073741824.0);
        } else {
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA host registration pointer lookup failed: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
        }
    } else {
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA host registration skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
    }
    return 1;
}
    ...
    if (fd >= 0) {
        ...
        if (getenv("DS4_CUDA_NO_DIRECT_IO") == NULL) {
            ...
            if (direct_fd >= 0) {
                ...
                if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
                    ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA model direct I/O enabled (align=%llu)\n",
                            (unsigned long long)g_model_direct_align);
                }
            } else if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
                ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: CUDA model direct I/O unavailable: %s\n", strerror(errno));
            }
        }
#endif
    }
...
extern "C" void ds4_gpu_print_memory_report(const char *label) {
    ...
    ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA memory report %s: free %.2f MiB total %.2f MiB\n",
    ...
    if (g_cublas_ready && n_tok > 1) {
        ...
        if (w_f16) {
            ...
            cublasStatus_t st = cublasGemmEx(g_cublas,
                                             ...
                                             CUBLAS_GEMM_DEFAULT);
            if (st == CUBLAS_STATUS_SUCCESS) return 1;
            ds4_gpu_log(DS4_GPU_LOG_ERROR, "ds4: cuBLAS q8 f16 matmul failed: status %d\n", (int)st);
...
extern "C" int ds4_gpu_attention_decode_heads_tensor(
    ...
    if (!cuda_attention_score_buffer_fits(n_comp)) {
        ...
        ds4_gpu_log(DS4_GPU_LOG_DEFAULT, "ds4: CUDA attention score buffer too small for %u compressed rows\n", n_comp);
        return 0;
    }
