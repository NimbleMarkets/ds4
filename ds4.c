...
static bool required_u64(const ds4_model *m, const char *key, uint64_t *out,
                         char *err, size_t errlen) {
    ds4_kv *kv = model_find_kv(m, key);
    if (!kv)
        return ds4_fail(err, errlen, "required metadata key is missing: %s", key);
    ...
    if (kv->type == GGUF_VALUE_UINT64) {
        if (!cursor_u64(&c, out)) return ds4_fail(err, errlen, "%s", c.error);
        return true;
    }
    if (kv->type == GGUF_VALUE_UINT32) {
        uint32_t v = 0;
        if (!cursor_u32(&c, &v)) return ds4_fail(err, errlen, "%s", c.error);
        *out = v;
        return true;
    }
    return ds4_fail(err, errlen, "metadata key has a non-integer type: %s", key);
}
...
static bool required_f32(const ds4_model *m, const char *key, float *out,
    ...
    if (kv->type == GGUF_VALUE_FLOAT32) {
        if (!cursor_read(&c, out, sizeof(*out))) return ds4_fail(err, errlen, "%s", c.error);
        return true;
    }
    ...
    return ds4_fail(err, errlen, "metadata key has a non-float type %u: %s", kv->type, key);
}
...
static bool tensor_expect_routed_expert(
    ...
    if (!tensor_is_routed_expert_type(t->type))
        return ds4_fail(err, errlen,
                        "tensor %.*s has type %u (%s), expected a routed expert quant type",
                        (int)t->name.len,
                        t->name.ptr,
                        t->type,
                        tensor_type_name(t->type));
...
static bool weights_validate_layout(const ds4_weights *w, char *err, size_t errlen) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if (ratio == 4) {
            ...
            if (!tensor_expect_layout(l->indexer_attn_q_b,        DS4_TENSOR_F16, 2, DS4_N_LORA_Q, index_q_dim, 0, err, errlen) ||
...
static bool validate_compress_ratio_metadata(const ds4_model *m, char *err, size_t errlen) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if (got != expected)
            return ds4_fail(err, errlen,
                            "unexpected DeepSeek4 compression ratio at layer %u: got %u, expected %u",
                            il, got, expected);
    }
    return true;
}
...
static bool config_expect_u32(const char *name, uint32_t got, uint32_t expected,
    ...
    return ds4_fail(err, errlen, "expected %s=%u for DeepSeek4 Flash, got %u",
                    name, expected, got);
}
...
static bool config_expect_f32(const char *name, float got, float expected,
    ...
    return ds4_fail(err, errlen, "expected %s=%.9g for DeepSeek4 Flash, got %.9g",
...
static bool config_expect_bool(const char *name, bool got, bool expected,
                                char *err, size_t errlen) {
    if (got == expected) return true;
    return ds4_fail(err, errlen, "expected %s=%s for DeepSeek4 Flash, got %s",
...
static bool config_validate_model(const ds4_model *m, char *err, size_t errlen) {
    ...
    if (!required_u32(m, "deepseek4.attention.sliding_window", &n_swa, err, errlen) ||
        !config_expect_u32("attention.sliding_window",     n_swa,              DS4_N_SWA, err, errlen) ||
    ...
    float rope_freq_base = 0.0f, rope_scale_factor = 0.0f;
    float rope_yarn_beta_fast = 0.0f, rope_yarn_beta_slow = 0.0f;
    ...
    if (!required_f32(m, "deepseek4.rope.freq_base", &rope_freq_base, err, errlen) ||
        ...
        !required_f32(m, "deepseek4.rope.scaling.factor", &rope_scale_factor, err, errlen) ||
        !config_expect_f32("rope.scaling.factor", rope_scale_factor, DS4_ROPE_SCALE_FACTOR, err, errlen) ||
        !required_f32(m, "deepseek4.rope.scaling.yarn_beta_fast", &rope_yarn_beta_fast, err, errlen) ||
        !config_expect_f32("rope.scaling.yarn_beta_fast", rope_yarn_beta_fast, DS4_ROPE_YARN_BETA_FAST, err, errlen) ||
        !required_f32(m, "deepseek4.rope.scaling.yarn_beta_slow", &rope_yarn_beta_slow, err, errlen) ||
...
static bool weights_bind(ds4_weights *w, const ds4_model *m, char *err, size_t errlen) {
    memset(w, 0, sizeof(*w));
    ...
    return weights_validate_layout(w, err, errlen);
}
...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    if (!model_open(&e->model, opt->model_path, graph_backend, true, NULL, 0)) {
        ds4_engine_close(e);
        *out = NULL;
        return 1;
    }
    ...
    if (!config_validate_model(&e->model, NULL, 0) ||
        !weights_bind(&e->weights, &e->model, NULL, 0)) {
        ds4_engine_close(e);
        *out = NULL;
        return 1;
    }
